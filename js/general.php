    ...
    if (valor !=""){
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#CUPON').val(response.code);
                $('#TIPOCUPON').val(response.type);
                $('#DESCUENTO').val(response.val);                    

                guardarCupon(response.code,response.type,response.val)
                
                let STotal = $('#checkout_subtotal').text();
                STotal = STotal.replace("$", "");
                $('#chEtiquetaCupon').text("Descuento ( "+response.code+" )")

                let Descuento = $('#chDescuento').text();
                Descuento = Descuento.replace("$", "");
                if (response.type == 'amount'){
                    Descuento = response.val * 1;
                }
                else{
                    Descuento = STotal * ( response.val / 100 );
                }

                $('#chDescuento').text(`$${Descuento.toFixed(2)}`)

                let TarjetaRegalo = $('#chTarjetaRegalo').text();
                TarjetaRegalo = TarjetaRegalo.replace("$", "");                

                let Total = $('#checkout_total').text();
                //Total = Total.replace("$", "");
                Total = STotal - Descuento - TarjetaRegalo;
                    ...
                    <div class="col-8">
                        <div class="card-body py-2 px-3">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                ...
                                <span class="fw-bold text-dark">$${formatCurrency(item.precio * item.cantidad)}</span>
                            </div>
                        </div>
                    </div>
                </div>
